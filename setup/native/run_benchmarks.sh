...
echo "Fetching time range for sampled trace..."
TRACE_TIME_STATS=$(run_query_silent "SELECT min(start_time), max(start_time) FROM spans WHERE trace_id = '${TRACE_ID}'")
export TRACE_TIME_MIN=$(echo "${TRACE_TIME_STATS}" | cut -f1)
export TRACE_TIME_MAX=$(echo "${TRACE_TIME_STATS}" | cut -f2)
echo "  TRACE_TIME_MIN: ${TRACE_TIME_MIN}"
echo "  TRACE_TIME_MAX: ${TRACE_TIME_MAX}"
