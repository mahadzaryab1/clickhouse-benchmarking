...
print_subheader "Retrieve Spans by Trace ID (with time range)"
query=$(cat "${SCRIPTS_DIR}/retrieve_spans_by_trace_id_with_time_range")
run_query "${query}" | column -t -s $'\t'
...
print_subheader "Search by Attribute"
query=$(cat "${SCRIPTS_DIR}/search_spans_by_attribute")
run_query "${query}" | column -t -s $'\t'
